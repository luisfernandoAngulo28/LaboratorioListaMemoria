...
void ListaP::eliminarPenultimoNodo() {
	if (longitud() < 2) {
		cout << "No hay suficientes nodos para eliminar el penúltimo." << endl;
		return;
	}
	direccionP penultimo = primero();
	direccionP antepenultimo = nullptr;
	// Avanzar hasta el penúltimo nodo
	while (penultimo->sig != nullptr && penultimo->sig->sig != nullptr) {
		antepenultimo = penultimo;
		penultimo = penultimo->sig;
	}

    // Enlazar el antepenúltimo con el último nodo
	antepenultimo->sig = penultimo->sig;

	// Liberar la memoria del penúltimo nodo
	delete penultimo;
...
void ListaP::insertarSiguienteDirPrimero(int dato) {
	if (vacia()) {
		cout << "La lista está vacía. No se puede insertar después del primero." << endl;
        return;
	}
    ...
    nuevoNodo->sig = PtrElementos->sig;
    PtrElementos->sig = nuevoNodo;
